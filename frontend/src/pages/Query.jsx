...
function Query() {
  ...
  const handleClearChat = async () => {
    ...
    try {
      ...
      await axios.delete("http://localhost:5000/api/query/history", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      Swal.fire({
        icon: "error",
        title: "Failed",
        text: error.response?.data?.message || "Unable to clear chat history.",
      });
    }
  };
  ...
  const handleAsk = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to process query");
    } finally {
      setLoading(false);
    }
  };
